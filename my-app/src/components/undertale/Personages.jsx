  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (docSnap.exists()) {
          setData(docSnap.data().personajes.filter(obj => obj.nombre == url)[0]);
          console.log(docSnap.data().personajes.filter(obj => obj.nombre == url)[0]);
        } else {
      ...
      <center>
        ...
        {url == undefined ? (<div>
          ...
          {characters.map((character) => (
            <Link to={`http://localhost:3000/Undertale/Personages/${character.name.replace(/ /g, '_')}`}>
          ...
          : (<div style={{ maxWidth: "600px", border: "4px solid rgb(255, 255, 255)", marginTop:"20px"}}>
            ...
            {/*<h3>HP {data.estadisticas.hp}/{data.estadisticas.hp}</h3>
            <h3>AT {data.estadisticas.ataque}</h3>
            <h3>DF {data.estadisticas.ataque}</h3>*/}
